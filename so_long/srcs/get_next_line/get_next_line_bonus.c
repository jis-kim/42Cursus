...
/*   Updated: 2021/05/27 19:43:19 by jiskim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*buf;
	static char	*buf_remain[OPEN_MAX];
	ssize_t		check_read;
	int			success;
	...
	if (!(buf = malloc(sizeof(char) * (BUFFER_SIZE + 1))))
		return (-1);
	if ((success = buf_split(&buf_remain[fd], line, 1)) != 0)
		return (ft_free(success, buf));
	while ((check_read = read(fd, buf, BUFFER_SIZE)) > 0)
	{
		buf[check_read] = '\0';
		if (!(buf_remain[fd] = ft_strjoin(buf_remain[fd], buf)))
			return (ft_free(-1, buf));
		if ((success = buf_split(&buf_remain[fd], line, check_read)) != 0)
			return (ft_free(success, buf));
	}
	if (check_read == 0)
		return (ft_free(buf_split(&buf_remain[fd], line, check_read), buf));
	return (check_read);
}
